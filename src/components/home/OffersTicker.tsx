...
export function OffersTicker() {
    return (
        <div className="bg-[#050505] text-[#FCD34D] py-1 md:py-2 overflow-hidden border-y border-[#FCD34D]/20 relative z-20 shadow-2xl group">
            ...
            <div className="flex animate-marquee whitespace-nowrap group-hover:[animation-play-state:paused] transition-all duration-300">
                ...
                {[...offers, ...offers, ...offers, ...offers].map((offer, index) => (
                    <div key={index} className="flex items-center gap-2 md:gap-3 mx-4 md:mx-8 select-none">
                        <div className="bg-[#FCD34D]/10 p-0.5 md:p-1 rounded-full border border-[#FCD34D]/20">
                            <offer.icon className="h-3 w-3 md:h-4 md:w-4 text-[#FCD34D]" />
                        </div>
                        ...
                        {/* Elegant Separator */}
                        <div className="ml-4 md:ml-8 h-6 md:h-8 w-[1px] bg-gradient-to-b from-transparent via-[#FCD34D]/30 to-transparent"></div>
                    </div>
                ))}
            </div>
