...
// Enable caching for GET requests; individual write operations remain non-cached
...
export async function GET(req: Request) {
    try {
        ...
        if (id) {
            ...
            if (!product) {
                return NextResponse.json(
                    ...
                    { status: 404, headers: { 'Cache-Control': 'max-age=60' } }
                );
            }
            return createCachedResponse({ success: true, product }, CacheConfig.PRODUCTS);
        }
        ...
        return createCachedResponse({ success: true, products }, CacheConfig.PRODUCTS);
    } catch (error) {
...
// Write operations don't need dynamic = "force-dynamic" anymore since GET is cached
// PUT/POST/DELETE will still work normally with request.json()
