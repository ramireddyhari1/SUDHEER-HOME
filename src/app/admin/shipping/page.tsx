...
export default function AdminShippingPage() {
    ...
    const fetchRates = async () => {
        try {
            // Fetch Display Rates
            const resRates = await fetch("/api/site-content?sectionId=shipping-rates");
            const jsonRates = await resRates.json();
            if (jsonRates.success && jsonRates.data && jsonRates.data.rates) {
                setRates(jsonRates.data.rates);
            } else {
                setRates([
            ...
            const resConfig = await fetch("/api/site-content?sectionId=checkout-shipping");
            const jsonConfig = await resConfig.json();
            if (jsonConfig.success && jsonConfig.data) {
                setCheckoutConfig({
                    standardCost: jsonConfig.data.standardCost || "50",
                    freeThreshold: jsonConfig.data.freeThreshold || "500"
                });
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            // Save Display Rates
            const resRates = await fetch("/api/site-content", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    sectionId: "shipping-rates",
                    content: { rates }
                })
            });

            // Save Checkout Config
            const resConfig = await fetch("/api/site-content", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    sectionId: "checkout-shipping",
