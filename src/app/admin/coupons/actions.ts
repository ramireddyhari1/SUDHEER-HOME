...
export async function createAndSendCoupon(data: CreateCouponData) {
    try {
        ...
        // Verify Token (Basic verification, assuming Middleware did the heavy lifting)
        // We can trust the middleware for basic presence, but specific action permission requires parsing
        // For now, just presence is a good start, or parse it if we want strict checks.
        // Given we installed 'jose', let's stick to basic cookie presence + jwt verification for robustness.
        const JWT_SECRET = process.env.JWT_SECRET || 'your-secret-key-change-in-production';
        try {
            // Basic verify without jose here to keep node runtime simple, or import jwt
            // Since this is Node runtime (Server Action), 'jsonwebtoken' works fine.
            const jwt = require('jsonwebtoken'); // dynamic import or top level
